...
class Config(object):
    def __init__(self):
        ...
        self.dropout = 0.5
        self.num_layers = 2
        self.save_path = './model_dict/gru_att_age_freeze.ckpt'
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.ad_id_embedding = nn.Embedding.from_pretrained(ad_id, freeze=True)
        self.product_id_embedding = nn.Embedding.from_pretrained(product_id, freeze=True)
        self.product_category_embedding = nn.Embedding(*config.product_category_embedding, padding_idx=0)
        self.advertiser_id_embedding = nn.Embedding.from_pretrained(advertiser_id, freeze=True)
        ...
        self.gru = nn.GRU(808, config.hidden_size, num_layers=config.num_layers, dropout=config.dropout, batch_first=True, bidirectional=True)
        self.weight_W = nn.Parameter(torch.Tensor(config.hidden_size*2, config.hidden_size*2))
